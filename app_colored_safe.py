...
def get_main_keyboard(user_id):
    keyboard = [
        [InlineKeyboardButton("💳 Account Balance", callback_data="main_balance", style="primary"), InlineKeyboardButton("🛒 Buy Number", callback_data="main_buy", style="success")],
        [InlineKeyboardButton("🌐 Set Country", callback_data="main_country", style="primary"), InlineKeyboardButton("📱 Set Service", callback_data="main_service", style="primary")],
        [InlineKeyboardButton("👤 Profile", callback_data="main_profile", style="primary"), InlineKeyboardButton("💵 Deposit", callback_data="main_deposit", style="success")]
    ]
    if user_id == ADMIN_ID:
        keyboard.append([InlineKeyboardButton("⚙️ Admin Panel", callback_data="main_admin", style="danger")])
...
async def handle_main_menu_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u_data.get("is_banned", False):
        await query.message.reply_text("❌ Apnar account-ti banned kora hoyeche.", reply_markup=ReplyKeyboardRemove())
        return
    ...
    if data == "main_country":
        ...
        await query.message.reply_text("🌐 **Desh nirbachon korun:**", reply_markup=ReplyKeyboardMarkup(country_kb, resize_keyboard=True))
        return
    ...
    if data == "main_service":
        ...
        await query.message.reply_text("📱 **Service nirbachon korun:**", reply_markup=ReplyKeyboardMarkup(service_kb, resize_keyboard=True))
        return
    ...
    if data == "main_buy":
        ...
        status_msg = await query.message.reply_text(f"⏳ `{country.upper()}` desher jonno `{service.upper()}` number kena hocche...")
        res = buy_vak_number(service, country)
        ...
        if isinstance(res, dict) and "tel" in res and "idNum" in res:
            ...
            active_orders[id_num] = {
                "user_id": user_id, "service": service, "cost": bot_rate,
                "phone": phone_num, "msg_id": sent_msg.message_id
            }
            try:
                await status_msg.delete()
            except Exception:
    ...
    if data == "main_admin" and user_id == ADMIN_ID:
        admin_kb = InlineKeyboardMarkup([
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_main_menu_callbacks, pattern="^main_(balance|buy|country|service|profile|admin)$"))
