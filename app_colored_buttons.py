...
def get_main_keyboard(user_id):
    keyboard = [
        [InlineKeyboardButton("💳 Account Balance", callback_data="main_balance", style="success"),
         InlineKeyboardButton("🛒 Buy Number", callback_data="main_buy_number", style="primary")],
        [InlineKeyboardButton("🌐 Set Country", callback_data="main_set_country", style="primary"),
         InlineKeyboardButton("📱 Set Service", callback_data="main_set_service", style="primary")],
        [InlineKeyboardButton("👤 Profile", callback_data="main_profile", style="primary"),
         InlineKeyboardButton("💵 Deposit", callback_data="main_deposit", style="danger")]
    ]
    if user_id == ADMIN_ID:
        keyboard.append([InlineKeyboardButton("⚙️ Admin Panel", callback_data="main_admin_panel", style="danger")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_subscribed(user_id):
        sub_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💳 Buy Subscription (30 Tk / 3 Days)", callback_data="buy_sub_start", style="success")]
        ])
        msg = (
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🛒 Buy Number":
        ...
        if isinstance(res, dict) and "tel" in res and "idNum" in res:
            ...
            inline_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("📩 Check Active OTP", callback_data=f"check_otp_{id_num}", style="primary")],
                [InlineKeyboardButton("❌ Cancel Number", callback_data=f"cancel_num_{id_num}", style="danger")]
            ])
    ...
    if text == "⚙️ Admin Panel" and user_id == ADMIN_ID:
        admin_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("👥 View All Users", callback_data="admin_view_users", style="primary")],
            [InlineKeyboardButton("🚫 Ban User", callback_data="admin_ban_start", style="danger"), InlineKeyboardButton("✅ Unban User", callback_data="admin_unban_start", style="success")],
            [InlineKeyboardButton("💵 Set WA Rate", callback_data="admin_rate_start", style="primary"), InlineKeyboardButton("➕ Add Balance", callback_data="admin_add_bal_start", style="success")]
        ])
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (data.startswith("main_") or data.startswith("country_") or data.startswith("service_")) and not is_subscribed(user_id):
        sub_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💳 Buy Subscription (30 Tk / 3 Days)", callback_data="buy_sub_start", style="success")]
        ])
        await query.message.reply_text("❌ Apnar subscription expired! Doya kore subscription kinun.", reply_markup=sub_kb)
        return
    ...
    if data == "main_balance":
    ...
    elif data == "main_set_country":
        country_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🇭🇰 Country: HK (Hong Kong)", callback_data="country_hk", style="primary"),
             InlineKeyboardButton("🇺🇸 Country: US (USA)", callback_data="country_us", style="primary")],
            [InlineKeyboardButton("🇷🇺 Country: RU (Russia)", callback_data="country_ru", style="primary"),
             InlineKeyboardButton("🇮🇳 Country: IN (India)", callback_data="country_in", style="primary")],
            [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu", style="danger")]
        ])
        await query.message.reply_text("🌐 **Desh nirbachon korun:**", parse_mode="Markdown", reply_markup=country_kb)
        return

    elif data in {"country_hk", "country_us", "country_ru", "country_in"}:
        code = data.split("_")[1]
        users_col.update_one({"user_id": user_id}, {"$set": {"selected_country": code}})
        await query.message.reply_text(
            f"✅ Country set hoyeche: `{code.upper()}`",
            parse_mode="Markdown",
            reply_markup=get_main_keyboard(user_id)
        )
        return

    elif data == "main_set_service":
        service_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🟢 Service: WA (WhatsApp)", callback_data="service_wa", style="primary"),
             InlineKeyboardButton("🔵 Service: TG (Telegram)", callback_data="service_tg", style="primary")],
            [InlineKeyboardButton("🟢 Service: GO (Google/Gmail)", callback_data="service_go", style="primary"),
             InlineKeyboardButton("🟣 Service: IM (Imo)", callback_data="service_im", style="primary")],
            [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu", style="danger")]
        ])
        await query.message.reply_text("📱 **Service nirbachon korun:**", parse_mode="Markdown", reply_markup=service_kb)
        return

    elif data in {"service_wa", "service_tg", "service_go", "service_im"}:
        code = data.split("_")[1]
        users_col.update_one({"user_id": user_id}, {"$set": {"selected_service": code}})
        await query.message.reply_text(
            f"✅ Service set hoyeche: `{code.upper()}`",
            parse_mode="Markdown",
            reply_markup=get_main_keyboard(user_id)
        )
        return
        ...
        await query.message.reply_text(
            "👇 Nicher menu theke option beche nin:",
            reply_markup=get_main_keyboard(user_id)
        )
        return

    elif data == "main_buy_number":
        u_data = get_or_create_user(user_id, query.from_user.full_name)
        country = u_data.get("selected_country", "hk")
        service = u_data.get("selected_service", "wa")
        bot_rate = get_rate(service)
        ...
        status_msg = await query.message.reply_text(
            f"⏳ `{country.upper()}` desher jonno `{service.upper()}` number kena hocche..."
        )
        res = buy_vak_number(service, country)
        ...
        if isinstance(res, dict) and "tel" in res and "idNum" in res:
            ...
            err_msg = res.get("error", "Stock Out") if isinstance(res, dict) else "Error"
            await query.message.reply_text(f"❌ **Number kena shombhov hoyni:** `{err_msg}`")
        return

    elif data == "main_deposit":
        return

    elif data == "main_admin_panel" and user_id == ADMIN_ID:
        admin_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("👥 View All Users", callback_data="admin_view_users", style="primary")],
            [InlineKeyboardButton("🚫 Ban User", callback_data="admin_ban_start", style="danger"),
             InlineKeyboardButton("✅ Unban User", callback_data="admin_unban_start", style="success")],
            [InlineKeyboardButton("💵 Set WA Rate", callback_data="admin_rate_start", style="primary"),
             InlineKeyboardButton("➕ Add Balance", callback_data="admin_add_bal_start", style="success")]
        ])
        await query.message.reply_text("🛠 **Admin Control Panel:**", reply_markup=admin_kb)
        return
...
async def sub_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bkash_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🌸 Bkash", callback_data="pay_bkash_sub", style="success")]
    ])
...
async def sub_screenshot_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Approve", callback_data=f"approve_sub_{user.id}", style="success"),
            InlineKeyboardButton("❌ Reject", callback_data=f"reject_sub_{user.id}", style="danger")
        ]
    ])
...
async def deposit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    payment_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💛 Binance Pay", callback_data="pay_binance", style="primary")]
    ])
    if update.callback_query:
        await update.callback_query.answer()
        await update.callback_query.message.reply_text("💳 **Payment Method select korunk:**", reply_markup=payment_kb)
    else:
...
async def deposit_screenshot_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Approve", callback_data=f"approve_dep_{user.id}_{amount}", style="success"),
            InlineKeyboardButton("❌ Reject", callback_data=f"reject_dep_{user.id}", style="danger")
        ]
    ])
...
def main():
    ...
    dep_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(deposit_start, pattern="^main_deposit$")
        ],
        states={
            WAITING_AMOUNT: [
